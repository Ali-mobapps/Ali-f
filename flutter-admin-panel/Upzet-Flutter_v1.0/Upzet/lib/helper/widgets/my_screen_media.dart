import 'package:upzet/helper/widgets/my_display_type.dart';
import 'package:upzet/helper/widgets/my_screen_media_type.dart';
...
class MyScreenMedia {
  ...
  static Map<MyScreenMediaType, T> getFilledMedia<T>(Map<MyScreenMediaType, T>? map, T defaultValue, [bool reversed = false]) {
  ...
  static Map<MyScreenMediaType, MyDisplayType> getDisplayDataFromString(String? string) {
    string ??= "";
    ...
    for (String item in data) {
      for (var type in MyScreenMediaType.values) {
        if (item.contains(type.className)) {
          MyDisplayType displayType = MyDisplayType.fromString(item.replaceAll("${type.className}-", ""));
          d[type] = displayType;
          break;
        }
      }
