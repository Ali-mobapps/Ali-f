...
import 'delivery_connector.dart';
...
class DeliveryService extends WooCommerceService {
  ...
  }) : _connector = DeliveryConnector(domain);

  final DeliveryConnector _connector;
  ...
  Future<Map<String, dynamic>> updateUserInfo(
    ...
    try {
      ...
      final response = await _connector.httpPost(
  ...
  Future<List<Order>> getDeliveryOrders({
    ...
    try {
      ...
      final response = await _connector.httpGet(
        endpoint.toUri()!,
        headers: {
  ...
  Future<DeliveryStat> getDeliveryStat({required String cookie}) async {
    try {
      var base64Str = EncodeUtils.encodeCookie(cookie);
  ...
  Future<List<WCFMNotification>> getNotifications({
    ...
    try {
      ...
      endpoint += '&platform=${ServerConfig().platform}';
      var response = await _connector.httpGet(
        endpoint.toUri()!,
        refreshCache: true,
      );
  ...
  Future<int> updateInfo(cookie, {firstName, lastName, phone, avatar}) async {
    try {
      ...
      var response = await _connector.httpPut(endpoint.toUri()!, body: data);
  ...
  Future<int> updateDeliveryOrder(cookie, orderId) async {
    try {
      ...
      var data = {'token': base64Str, 'order_id': orderId};
  ...
  Future<List<OrderNote>?> getOrderNotes(String orderId) async {
    ...
    try {
      ...
      var response = await _connector.httpGet(endpoint.toUri()!);
  ...
  Future<int> setOffTime(String cookie, bool isOffline) async {
    try {
      var base64Str = EncodeUtils.encodeCookie(cookie);
      var data = {'token': base64Str, 'is_available': isOffline.toString()};
