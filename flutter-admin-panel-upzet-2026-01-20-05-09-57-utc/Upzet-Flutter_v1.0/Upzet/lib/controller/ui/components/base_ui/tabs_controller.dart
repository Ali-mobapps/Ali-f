...
class TabsController extends MyController {
  var defaultIndex = 0, fullWidthIndex = 0, backgroundIndex = 0, borderedIndex = 0, softIndex = 0, customIndex1 = 0, customIndex2 = 0;

  late final TickerProvider tickerProvider;

  late TabController defaultTabController = TabController(length: 3, vsync: tickerProvider, initialIndex: defaultIndex);
  late TabController fullWidthTabController = TabController(length: 3, vsync: tickerProvider, initialIndex: fullWidthIndex);
  late TabController backgroundTabController = TabController(length: 3, vsync: tickerProvider, initialIndex: backgroundIndex);
  late TabController borderedTabController = TabController(length: 3, vsync: tickerProvider, initialIndex: borderedIndex);
  late TabController softTabController = TabController(length: 3, vsync: tickerProvider, initialIndex: softIndex);
  late TabController customTabController1 = TabController(length: 3, vsync: tickerProvider, initialIndex: customIndex1);
  late TabController customTabController2 = TabController(length: 3, vsync: tickerProvider, initialIndex: customIndex2);
